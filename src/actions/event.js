...
export const getEvents = () => async (dispatch) => {
  ...
  export const attendEvent = (id) => async (dispatch) => {
    console.log('Attend');
    try {
      const res = await axios.put(`/events/attendance/${id}`);//Bu fonksiyon backenddeki events in attended bölümüne user ın id sini koyacak
  
      dispatch({
        type: UPDATE_ATTENDANCY,
        payload: { id, attendances: res.data },//attendances içine bu payload ile id yi koyacak
      });
    } catch (err) {
      dispatch({
        type: EVENT_ERROR,
